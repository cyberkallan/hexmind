...
class HexMind:
    ...
    def _first_time_setup(self):
        ...
        rapid_key = Prompt.ask(
            "\n  [bold cyan]RapidAPI Key (Optional)[/bold cyan]\n"
            "  [dim]Enter your X-RapidAPI-Key to enable the Omni-Agent for OSINT/Weather/Phone lookups, or press Enter to skip.[/dim]",
            default=""
        ).strip()
        
        if rapid_key:
            # Save it to keys.json
            keys_file = CONFIG_DIR / "keys.json"
            try:
                keys_data = json.loads(keys_file.read_text()) if keys_file.exists() else {}
                keys_data["rapidapi_key"] = rapid_key
                keys_file.write_text(json.dumps(keys_data, indent=4))
                console.print(f"  [green]RapidAPI key saved to keys.json.[/green]")
            except Exception as e:
                console.print(f"  [red]Failed to save RapidAPI key: {e}[/red]")
