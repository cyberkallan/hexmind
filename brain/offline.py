...
GREETINGS = {"hello","hi","hey","sup","yo","hiya","howdy","what's up","wassup",
             "namaste","namaskar","vanakkam","salam","namaskaram","sugham",
             "good morning","good evening","good night","mash","machane"}
...
CHITCHAT = {
    ...
    ("how are you","how r u","you ok","you good","sugham aano","eppadi irukke"):
        lambda n: random.choice(["Running great! Brain loaded 🧠 What are we hacking?","Njan sugham aanu, mash! 😄 Enth cheyyanum?","All good bro! Ready to roll. What's the target?","Doing well — ready to hack something (ethically 😄). Ask away!"]),
    ("thank","thanks","thx","ty","cheers","nanni","nanri"):
        lambda n: random.choice(["Anytime bro! 😎","No problem! Keep hacking.","Happy to help, {n}! Adipoli! 🔥".format(n=n),"Santhosham aanu, {n}! 💪".format(n=n)]),
    ("bye","goodbye","later","cya","exit","pinne kaanaam","poi varukaayam"):
        lambda n: random.choice([f"Later, {n}! Stay safe out there 👋",f"Pinne kaanaam, {n}! Take care mash! 🙏",f"See you, {n}! Happy hacking! 🔥"]),
    ("joke","tell me a joke","make me laugh"):
        lambda n: random.choice([
            ...
            "Why don't hackers use Windows?\nBecause they can't break what's already broken! 🤣",
        ]),
    ...
    ("good morning","good evening","shubha dina","suprabhatam"):
        lambda n: random.choice([f"Good morning {n}! ☀️ Ready for some hacking today?",f"Morning mash! ☕ What's on the agenda?",f"Suprabhatam, {n}! Let's make today productive 💪"]),
    ("namaste","namaskar","vanakkam","salam","namaskaram"):
        lambda n: random.choice([f"Namaste {n}! 🙏 How can I help you today?",f"Vanakkam, {n}! 🙏 Enth cheyyanum?",f"Salam {n}! Ready to work together 💪"]),
}
...
AGENT = [
    ...
    (r'\binstall\s+([^\s]+)',
     lambda m: f"__ASK_AI_INSTALL__ {m.group(1).strip()}"),
    (r'\b(?:orchestrate|meta|hex)\s+(.+)',
     lambda m: f"__META_AGENT__ {m.group(1).strip()}"),
    (r'\b(?:dag|workflow)\s+(.+)',
     lambda m: f"__DAG_AGENT__ {m.group(1).strip()}"),
    (r'\bresearch\s+(.+)',
     lambda m: f"__DEEP_RESEARCH__ {m.group(1).strip()}"),
    (r'\bsurf\s+(.+)',
     lambda m: f"__FILE_SURFER__ {m.group(1).strip()}"),
    (r'\b(?:scrape|extract)\s+(https?://[^\s]+)',
     lambda m: f"__WEB_SCRAPE__ {m.group(1).strip()}"),
    (r'\bbounty\s+(.+)',
     lambda m: f"__BOUNTY_HUNTER__ {m.group(1).strip()}"),
    (r'\bstop bounty\s+(.+)',
     lambda m: f"__STOP_BOUNTY__ {m.group(1).strip()}"),
    (r'\b(?:mutate|scramble|bypass)\s+(.+)',
     lambda m: f"__EXPLOIT_MUTATOR__ {m.group(1).strip()}"),
    (r'\b(?:vision|ui attack)\s+(https?://[^\s]+)',
     lambda m: f"__VISION_ATTACKER__ {m.group(1).strip()}"),
    (r'\bswarm\s+(.+)',
     lambda m: f"__SWARM_AGENT__ {m.group(1).strip()}"),
